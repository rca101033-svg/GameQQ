...
- 點擊六角地圖節點：消耗 1 點 AP，推進時間並消耗飽食與水分。
- 點擊手牌：使用射擊、補給或繃帶卡牌；卡牌會直接影響戰鬥與生存數值。
- 點擊「結束回合」：恢復 AP，若敵人仍在場會受到反擊。
- 點擊「休息治療」：在安全屋恢復生命、清除感染並回到清晨。
- 點擊「返回安全屋並存檔」：結算目前補給，模擬安全撤離循環。

目前版本是可匯出的 playable prototype，後續可在此基礎上加入真正的卡組抽取、六角路徑限制、戰鬥動畫與音效素材。
